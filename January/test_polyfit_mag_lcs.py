...
def polyfit_lc_mags(ant_name, df, fit_order, df_bands, trusted_band, min_band_dps, fit_MJD_range, extrapolate, b_colour_dict, plot_polyfit = False):
    """
    ...
    # =========================================================================================================================================================================================================
    # =========================================================================================================================================================================================================
    def fudge_polyfit_magerr(real_b_df, polyfit_mag, scaled_reference_MJDs, MJD_scaledown):
        """
        ...
        """
        def fudge_error_formula(mean_err, mjd_dif, mag):
            x = abs( mag * (mjd_dif / 10) )
        ...
        rb_magerr = np.array(real_b_df['magerr'].copy()) # an array of the band's real L_rf errors
        ...
        for i, mag in enumerate(polyfit_mag): # iterate through each L_rf value calculated by evaluating the polyfit at the reference band's MJDs
            ...
            sc_closest_20_err = [rb_magerr[j] for j in closest_20_idx] #  a list of the wm_L_rf_err values for the 20 closest datapoints to our interpolated datapoint
            ...
            fudge_poly_magerr = fudge_error_formula(sc_mean_L_rf_err, closest_MJD_diff, mag)
            #poly_L_rf_er =  sc_poly_L_rf_er / L_rf_scaledown # to scale L_rf (fudged) error
            
            if fudge_poly_magerr < 0.0:
                print(mag, fudge_poly_magerr)
            #fudge_err_list.append(fudge_poly_magerr)
            fudge_err_list.append(0.0001) ################################################################################################# CHANGE THIS!!!!!!!!!!!!!!!!!!
    ...
    polyfit_ref_lc_df = pd.DataFrame({'MJD': ref_band_MJD, 
                                      'mag': np.array(ref_band_df['mag'].copy()), 
                                      'magerr': np.array(ref_band_df['magerr'].copy()),
                                      'band': list(ref_band_df['band'].copy()), 
                                      'em_cent_wl': list(ref_band_df['em_cent_wl'].copy())
                                      })
    
    columns = ['band', 'poly_MJD', 'poly_mag', 'red_chi', 'red_chi_1sig', 'chi_sigma_dist'] # allows us to plot the polyfit as a line using plt.plot() and the interpolated values.                                                                 
    ...
    for i, b in enumerate(df_bands):
        ...
        if b == trusted_band: # we don't need a polyfit of the trusted band because we're just evaluating the polyfits of all other bands at the trusted_band's MJDs
            if plot_polyfit == True:
                ...
                plt.errorbar(b_df['wm_MJD'], b_df['mag'], yerr = b_df['magerr'], c = b_colour, fmt = 'o', linestyle = 'None', markeredgecolor = 'k', 
        ...
        poly_coeffs = np.polyfit(MJD_scaled, b_lim_df['mag'], deg = fit_order)
        ...
        poly_plot_mag = polynomial_fit(poly_plot_MJD_scaled)
        ...
        poly_mag_for_chi = polynomial_fit(poly_MJD_for_chi_scaled) 
        #poly_L_rf_for_chi = poly_L_rf_for_chi_scaled / L_rf_scalefactor
        red_chi, red_chi_1sig = chisq(y_m = poly_mag_for_chi, y = b_lim_df['mag'], yerr = b_lim_df['magerr'], M = (fit_order + 1))
        ...
        interp_mag  = polynomial_fit(interp_MJD_scaled)
        ...
        interp_magerr= fudge_polyfit_magerr(b_df, interp_mag, interp_MJD_scaled, MJD_scaleconst)
        interp_b_df = pd.DataFrame({'MJD': interp_MJD, 
                                    'mag': interp_mag, 
                                    'magerr': interp_magerr, 
        ...
        plot_polyline_row[:] = [b, poly_plot_MJD, poly_plot_mag, red_chi, red_chi_1sig, chi_sigma_dist]
        plot_polyline_df.loc[plot_polyline_rowno] = plot_polyline_row
        # PLOTTING ====================================================================================================================================================================
        if plot_polyfit == True:
            b_colour = b_colour_dict[b]

            plt.errorbar(b_df['wm_MJD'], b_df['mag'], yerr = b_df['magerr'], fmt = 'o', markeredgecolor = 'k', markeredgewidth = '1.0', linestyle = 'None', 
                         label = b, c = b_colour)
            plt.plot(poly_plot_MJD, poly_plot_mag, c = b_colour, label = f'red chi = {red_chi:.3f}  \n +/- {red_chi_1sig:.3f}')
            plt.errorbar(interp_b_df['MJD'], interp_b_df['mag'], yerr = interp_b_df['magerr'], fmt = '^', c = b_colour, markeredgecolor = 'k', markeredgewidth = '0.5', 
...
#interp_lc, plot_polyfit_df = polyfit_lc2(ANT_name, ANT_df, fit_order = 5, df_bands = bands_for_BB, min_band_dps = 10, trusted_band = reference_band, fit_MJD_range = polyfit_MJD_range,
#                        extrapolate = False, b_colour_dict = band_colour_dict, plot_polyfit = True)

poly_interp_df, plot_polyfit_df = polyfit_lc_mags(ANT_name, ANT_df, fit_order = 9, df_bands = ANT_bands, trusted_band = reference_band, min_band_dps = 5, 
                                                  fit_MJD_range = MJDs_for_fit[ANT_name], extrapolate = False, b_colour_dict = band_colour_dict, plot_polyfit = True)
