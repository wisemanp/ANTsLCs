...
def fit_BB_across_lc(interp_df, curvefit, brute, brute_gridsize, brute_param_sigma = 1, BB_R_min = 1e13, BB_R_max = 1e19, BB_T_min = 1e3, BB_T_max = 1e7):
    ...
    for MJD in tqdm(mjd_values, desc = 'Progress BB fitting each MJD value', total = len(mjd_values), leave = False):
        ...
        if MJD_no_bands <= 1: # if there's <= 2 bands present for a particular MJD, don't bother fitting a BB 
        ...
        BB_fit_results.at[df_row_index] = BB_result_row # adding the array of results from this MJD to the BB results dataframe
...
class fit_BB_across_lightcurve:
    ...
    def BB_brute(self, MJD, MJD_df):
        ...
        param_grid = np.meshgrid(R_values, T_values)
        # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # add the result to the results row which will be appended to the results dataframe
