...
class fit_BB_across_lightcurve:
    ...
    def get_individual_BB_fit_MJDs(self, no_MJDs = 12):
        """
    ...
    def plot_individual_BB_fits(self, band_colour_dict):
        if self.indiv_plot_MJDs is not None:
            fig, axs = plt.subplots(4, 3, figsize = (16, 7.5), sharex = True)
            axs = axs.flatten()
            legend_dict = {}
            for i, MJD in enumerate(self.indiv_plot_MJDs):
                ...
                for b in MJD_df['band'].unique():
                    ...
                    legend_dict[b] = h[0]
                title1 = f'Days since peak = {d_since_peak:.0f} d'+ r'  $\chi_{\nu}$ sig dist = '+f'{self.BB_fit_results.loc[MJD, "brute_chi_sigma_dist"]:.2f}'# \n'
                title2 = r'T = '+f"{self.BB_fit_results.loc[MJD, 'cf_T_K']:.1e} +/- {self.BB_fit_results.loc[MJD, 'cf_T_err_K']:.1e} K \n"
                title3 = r'R = '+f"{self.BB_fit_results.loc[MJD, 'cf_R_cm']:.1e} +/- {self.BB_fit_results.loc[MJD, 'cf_R_err_cm']:.1e} cm"
            ...
            if self.save_indiv_BB_plot == True:
                savepath = f"C:/Users/laure/OneDrive/Desktop/YoRiS desktop/YoRiS/plots/BB fits/random/Bham_{self.ant_name}_subplot_indiv_BB_fits.png"
