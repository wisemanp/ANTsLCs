...
from normal_code.plotting_preferences import ANT_sim_redshift_upper_lim_dict, ANT_proper_redshift_dict
...
def blackbody_L_lam_rf(lam_cm, R_cm, T_K):
    """
    Planck's blackbody formula modified to give luminosity density (per unit wavelength) in units ergs/s/Angstrom
    ...
    RETURNS
    --------------
    L: blackbody luminosity density (per unit wavelength) for the wavelength input. Units: ergs/s/Angstrom
    """
...
def sim_ANY_LSST_lc(SED_filename, SED_filepath, max_sig_dist, max_chi_N_equal_M, size, model, mysurvey, tstart, tstop, no_plots_to_save, ANT_max_sim_redshift_dict, 
    ...
    """
    This function takes a csv file containing the SED evolution of a transient and simulates more light curves based on this SED evolution
    ...
    max_sig_dist: (float) the maximum value of the reduced chi squared sigma distance of an SED which is still considered to be a good fit. SEDs with sigma distances
                    above this value will be considered to be bad fits and will not be used for simulation. I used 3 for my dissertation.
    ...
    max_chi_N_equal_M: (float) We include fits where N=M in our simulations. Since they have no defined value of red chi or red chi dig dist, set a limit for the chi to 
                        consider the fit 'good'. I used 0.1 for my dissertation
    ...
    no_plots_to_save: (float) the number of simulated lightcurves you would like to save. (I usually have this quite a lot less than size, since I don't 
    want to save every single sinumated light curve, and also some of the simulated objects might not get observed by LSST)

    ANT_max_sim_redshift_dict: (dict) contains the maximum redshift to simulate each ANT to, where the keys are the ANT names and the values are the max redshift values.

    ANT_luminosity_dist_cm_dict: (dict) contains the luminosity distance of each ANT calculated in cm ASSUMING A COSMOLOGY, where the keys are the ANT names
    and the values are the luminosity distances in cm. 

    plot_skysurvey_inputs: (bool) if you want to plot and save the SED parameters than you are inputting into skysurvey to make sure they're doing what you want (for sanity checking)

    plot_SED_results: (bool) if you want to plot and save what the individual SEDs look like. This will be a single plot with many SEDs plotted onto it. (for sanity checking)
    ...
    ANT_z = ANT_max_sim_redshift_dict[ANT_name]
    ...
    if ANT_name == 'PS1-10adi': # In my diss, I chose to use the median D_{\sigma_\chi} + 1 MAD as the maximum sigma distance for these objects's sigma distance limits since they are not well fit by any SED model
        max_sig_dist = 19.736 + 13.89 # Median D_{\sigma_\chi} + 1 MAD
    ...
    if 'SBB' in SED_filename:
        ...
        ANT_SED_N_equals_M = ANT_SED_N_equals_M[ANT_SED_N_equals_M['brute_chi'] <= max_chi_N_equal_M].copy() # take the fits where N=M and chi^2 <= 0.1 or whatevr limit is input
        ...
        if plot_skysurvey_inputs:
            ...
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_SBB/{ANT_name}_INPUT_PARAMS_SBB.png", dpi = 300)
    ...
    # POWER LAW TRANSIENT SIMULATION
        ...
        pl_transient = skysurvey.TSTransient.from_draw(size = size, model = model, template = pl_source, tstart=tstart, tstop=tstop, zmax = ANT_max_simulation_z)
        ...
        if plot_skysurvey_inputs:
            ...
            fig.suptitle(f"Input params for {ANT_name}'s SBB SED fit", fontweight = 'bold')
            fig.tight_layout()
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_PL/{ANT_name}_INPUT_PARAMS_PL.png", dpi = 300)


    # ================================================================================================================================================================================================================================================================================================
    # DOUBLE-BLACKBODY TRANSIENT SIMULATION
        ...
        # filter for the good SED fits only 
        ANT_good_SEDs = ANT_SED_df[ANT_SED_df['cf_chi_sigma_dist'].abs() <= max_sig_dist].copy() # take the good fits where N>M
        ANT_SED_N_equals_M = ANT_SED_df[ANT_SED_df['cf_chi_sigma_dist'].isna()].copy() # take all fits where N=M
        ANT_SED_N_equals_M = ANT_SED_N_equals_M[ANT_SED_N_equals_M['cf_chi'] <= max_chi_N_equal_M].copy() # take the fits where N=M and chi^2 <= 0.1
        ...
        dbb_transient = skysurvey.TSTransient.from_draw(size = 200, model = model, template = dbb_source, tstart="2026-04-10", tstop="2031-04-01", zmax = ANT_max_simulation_z)
        ...
        if plot_skysurvey_inputs:
            ...
            fig.tight_layout()
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_DBB/{ANT_name}_INPUT_PARAMS_DBB.png", dpi = 300)
    ...
    if plot_SED_results:
        ...
        if SED_label == 'SBB':
            ...
            fig.tight_layout()
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_SBB/{ANT_name}_GENERATE_INPUT_SBB_SEDs.png", dpi = 300)
            ...
            fig.tight_layout()
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_PL/{ANT_name}_GENERATE_INPUT_PL_SEDs.png", dpi = 300)
            ...
            fig.tight_layout()
            plt.savefig(f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_DBB/{ANT_name}_GENERATE_INPUT_DBB_SEDs.png", dpi = 300)
    ...
    # for 2, 4  full page plot ======================================================================================
    for j, index in enumerate(dset.obs_index):
        #if j in [0, 9, 18, 27, 36, 45, 54, 63, 72, 81, 90, 99, 108, 117, 126, 135, 144]:
        if j in [0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128]:
            a = 0
            fig, axs = plt.subplots(4, 2, figsize = (8.2, 11.6)) # (16, 12) # (16, 7.5)
            handles = []
            labels = []
            axs = axs.ravel()
            titlefontsize = 18#24
            ...
            fig.subplots_adjust(left = 0.18, right = 0.85, hspace = 0.45, wspace = 0.45, bottom = 0.1) # wspace = 0.32
        ...
        ax.tick_params(axis='both', labelsize = 10) #15
        ...
        ax.set_title(subplot_title, fontweight = 'bold', fontsize = 11)# 18 
        ...
        if j in [7, 15, 23, 31, 39, 47, 55, 63, 71, 79, 87, 95, 103, 111, 119, 127, 135]:
            ...
            base_savepath = f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_{SED_label}/FINAL_SUBPLOT_{ANT_name}_SED_{SED_label}_lc_sim"
            ...
            plt.legend(handles, labels, loc = 'upper right', fontsize = 10, bbox_to_anchor = (1.5, 5.5)) #(1.38, 4.0) # fontsize = 15
    ...
    # this plots one light curve in the format output directly from skysurvey, with the flux in photons/s/cm^2, scaled by the AB magnitude system, 
    # and then the light curve in flux density in ergs/s/cm^2/Angstrom, also scaled by the AB magnitude system
    i_before_run = None 
    for j, index in enumerate(dset.obs_index):
        ...
        # save the plot - since the result of each run of the code is different, make sure to save each plot with a different name (adding  anumber at the end) os we don't overwrite the previously saved plots
        base_savepath = f"/media/data3/lauren/YoRiS/final_simulated_lcs/{ANT_name}/LSST_{SED_label}/{ANT_name}_SED_{SED_label}_lc_sim"
        i = 1
...
# with path, you can use either the path to a single SED fit file (in which case you set folder = False), 
# or the path to a folder containing multiple SED fit files (in which case you set folder = True). Running the code on an entire folder is
# useful because it will only load in the LSST file once, then loop through each SED fit file in the folder, instead of loading in the
# LSST file for each SED fit file, because loading in the LSST file takes 5+ minutes. 
path = "/media/data3/lauren/YoRiS/SED_fits_for_simulation"
#path = "/media/data3/lauren/YoRiS/SED_fits_for_simulation/ZTF22aadesap_UVOT_guided_PL_SED_fit_across_lc_new.csv"
folder = True
...
if not folder:
    ...
    sim_ANY_LSST_lc(SED_filename = SED_filename, SED_filepath = path, max_sig_dist = max_sig_dist, max_chi_N_equal_M = max_chi_N_equal_M, size = size, model = _MODEL, mysurvey = LSST_survey, 
                        tstart = tstart, tstop = tstop, no_plots_to_save = no_plots_to_save, ANT_max_sim_redshift_dict = ANT_sim_redshift_upper_lim_dict, 
    ...
    for SED_filename in os.listdir(path = "/media/data3/lauren/YoRiS/SED_fits_for_simulation"):
        full_filepath = os.path.join(path, SED_filename)
        ANT_name = SED_filename.split('_')[0] # this splits up the string that is the file name by the underscores, then we take all of the file name up until the first underscore
        ANT_D_cm = ANT_luminosity_dist_cm_dict[ANT_name]
        ANT_SED_df = pd.read_csv(full_filepath, delimiter = ',', index_col = 'MJD')
        #print(ANT_name)
        #print(SED_filename)
        #print(ANT_SED_df)
        #print()

        sim_ANY_LSST_lc(SED_filename = SED_filename, SED_filepath = full_filepath, max_sig_dist = max_sig_dist, max_chi_N_equal_M = max_chi_N_equal_M, size = size, model = _MODEL, mysurvey = LSST_survey, 
                        tstart = tstart, tstop = tstop, no_plots_to_save = no_plots_to_save, ANT_max_sim_redshift_dict = ANT_sim_redshift_upper_lim_dict, 
                        ANT_luminosity_dist_cm_dict = ANT_luminosity_dist_cm_dict, time_spline_degree = time_spline_degree, wavelength_spline_degree = wavelength_spline_degree,
                        plot_skysurvey_inputs = True, plot_SED_results = True)