...
if SED_plots == 'usual':
    ...
    #for idx in [13]:
    for idx in [transient_names.index('ZTF22aadesap')]: #   ZTF22aadesap  ZTF18aczpgwm
        ...
        no_indiv_SED_plots = 2 # current options are 24, 20, 12 (OR 2 if using the presentation functions)
...
if SED_plots == 'compare_SEDs':
    ...
    #for idx in range(len(transient_names)):
    #for idx in [3]:
    for idx in [transient_names.index('ZTF20abodaps')]: # ZTF22aadesap
        ANT_name = transient_names[idx]
        interp_lc= interp_df_list[idx]
        ANT_bands = list_of_bands[idx]
        print('=================================================================================================')
        ...
        if ANT_name in ['ZTF19aailpwl', 'ZTF20acvfraq', 'ZTF22aadesap', 'ASASSN-17jz', 'ASASSN-18jd']: # FOR THE OBJECTS WITH UV DATA, FIT ALL SED TYPES + COMPARE. KEEP COMPARISON BETWEEN UV VS OPTICAL ONLY EPOCHS
            ...
            for i, SED_type in enumerate(['single_BB', 'double_BB', 'power_law']):
                ...
                label1 = fr'$\mathbf{{{SED_label}}}$ '+'\n'+fr'Median $D_{{\sigma_\chi}}$ = {all_lc_median:.1f}'+'\n'+fr'MAD $D_{{\sigma_\chi}}$ = {all_lc_MAD:.1f}'
                ...
                label2 = fr'$\mathbf{{{SED_label}}}$ '+'\n'+fr'Median $D_{{\sigma_\chi}}$ = {no_UVOT_median:.1f} '+'\n'+fr'MAD $D_{{\sigma_\chi}}$ = {no_UVOT_MAD:.1f}'
                ...
                label3 = fr'$\mathbf{{{SED_label}}}$ '+'\n'+fr'Median $D_{{\sigma_\chi}}$ = {rise_peak_median:.1f} '+'\n'+fr'MAD $D_{{\sigma_\chi}}$ = {rise_peak_MAD:.1f}'
                ...
                label4 = fr'$\mathbf{{{SED_label}}}$ '+'\n'+fr'Median $D_{{\sigma_\chi}}$ = {UVOT_median:.1f} '+'\n'+fr'MAD $D_{{\sigma_\chi}}$ = {UVOT_MAD:.1f}'
            ...
            subfontsize = 15
            ax1.set_title('Entire lightcurve', fontweight = 'bold', fontsize = subfontsize)
            ax2.set_title('Phases without UV data', fontweight = 'bold', fontsize = subfontsize)
            ax3.set_title(r'Phases $\mathbf{\leq100}$ days', fontweight = 'bold', fontsize = subfontsize)
            ax4.set_title('Phases with UV data', fontweight = 'bold', fontsize = subfontsize)
