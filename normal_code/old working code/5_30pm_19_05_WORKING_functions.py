...
class fit_SED_across_lightcurve:
    ...
    def format_sci_with_uncertainty(value, upper, lower):
        ...
        return rf"$\mathbf{{{base:.2g}^{{+{upper_scaled:.2g}}}_{{-{lower_scaled:.2g}}} \times 10^{{{exp}}}}}$"#rf"${base:.2g}^{{+{upper_scaled:.2g}}}_{{-{lower_scaled:.2g}}} \times 10^{{{exp}}}$" #rf"${base:.2g}^{{+{upper_scaled:.2g}}}_{{-{lower_scaled:.2g}}} \times 10^{{{exp}}}$"
    ...
    def plot_individual_BB_fits(self, band_colour_dict, band_marker_dict):
        ...
        if self.indiv_plot_MJDs is not None:
            ...
            for i, MJD in enumerate(self.indiv_plot_MJDs):
                ...
                #print(self.BB_fit_results.loc[MJD, 'brute_T_K'], type(self.BB_fit_results.loc[MJD, 'brute_T_K']))
                t_label = self.format_sci_with_uncertainty(value = (self.BB_fit_results.loc[MJD, 'brute_T_K']), upper = self.BB_fit_results.loc[MJD, 'brute_T_err_upper_K'], lower = self.BB_fit_results.loc[MJD, 'brute_T_err_lower_K'])
                title2 = t_label + ' K\n'
            ...
            fig.suptitle(suptitle, fontweight = 'bold', fontsize = titlefontsize)
            fig.legend(legend_dict.values(), legend_dict.keys(), loc = 'upper right', fontsize = 8.5, bbox_to_anchor = (1.02, 0.95))
            fig.subplots_adjust(top=0.875,
                                bottom=0.07,
                                left=0.18,
                                right=0.85,
