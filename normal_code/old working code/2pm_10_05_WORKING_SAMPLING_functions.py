...
class fit_SED_across_lightcurve:
    ...
    def double_BB_brute(self, MJD, MJD_df, R1_sc_min, R1_sc_max, T1_min, T1_max, R2_sc_min, R2_sc_max, T2_min, T2_max, cf_chi, cf_chi_sigma_dist, cf_red_chi, cf_T1, cf_R1, cf_T2, cf_R2):
        ...
        if len(chi_flat) == 0:
            ...
            #self.BB_fit_samples.loc[(MJD, self.error_sampling_size), sampled_row_dict.keys()] = pd.Series(sampled_row_dict) # if we don't have any values within the delchi = 2.3, just save the actual curve_fit params and go with that
