...
class fit_SED_across_lightcurve:
    ...
    def get_individual_BB_fit_MJDs(self):
        ...
        elif self.no_indiv_SED_plots == 2:
            phase_1 = -49.38 #-43.00 #-49.38 #24
            phase_2 = -20.44 #-22.68 #126

            idx_1 = (self.interp_df['d_since_peak'] - phase_1).abs().idxmin()
            idx_2 = (self.interp_df['d_since_peak'] - phase_2).abs().idxmin()

            MJD_1 = self.interp_df['MJD'].iloc[idx_1]
            MJD_2 = self.interp_df['MJD'].iloc[idx_2]
            self.indiv_plot_MJDs = np.array([MJD_1, MJD_2])
    ...
    def plot_individual_BB_fits(self, band_colour_dict, band_marker_dict):
        ...
        tick_labelsize = 17.5
        y_scalefactor = 1e39
        y_scalefactor_exponent = 39

        if self.indiv_plot_MJDs is not None:
            fig, axs = plt.subplots(nrows, ncols, figsize = (9, 13), sharex = True)
            legend_dict = {}
            ...
            for i, MJD in enumerate(self.indiv_plot_MJDs):
                ...
                subplot_title = f'Phase = {d_since_peak:.0f}'
                ...
                h_BB, = ax.plot(plot_wl*1e8, (plot_BB_L / y_scalefactor), c = 'k', label = title2 + title3)
                ax.grid(True)
                ...
                for b in MJD_df['band'].unique():
                    ...
                    h = ax.errorbar(b_df['em_cent_wl'], (b_df['L_rf'] / y_scalefactor), yerr = (b_df['L_rf_err'] / y_scalefactor), fmt = b_marker, c = b_colour, label = b, mec = 'k', mew = '1.4', markersize = markersize)
                    legend_dict[b] = h[0]
                
                #leg = ax.legend(handles = [h_BB], labels = [title2 + title3], prop = {'weight': 'bold', 'size': '7'})#, loc='lower left', bbox_to_anchor=(0.05, 0.05))
                #leg.get_frame().set_alpha(0.6)
                ax.set_title(subplot_title, fontsize = ax_titlesize, fontweight = 'bold')
            

            
            suptitle = f"Single-Blackbody SED Fits at Different\nEpochs of {self.ant_name}'s Lightcurve"
            fig.supxlabel(r'Emitted Wavelength [$\mathbf{\AA}$]', fontweight = 'bold', fontsize = axis_fontsize)
            fig.supylabel(f'Luminosity Density [$\mathbf{{10^{{{y_scalefactor_exponent}}}}}$ erg ' + r's$\mathbf{^{-1} \, \AA^{-1}}$]', fontweight = 'bold', fontsize = axis_fontsize)
            fig.suptitle(suptitle, fontweight = 'bold', fontsize = titlefontsize)
            ...
            if self.save_indiv_BB_plot == True:
                if self.guided_UVOT_SED_fits:
                    if self.no_indiv_SED_plots == 2:
                        savepath = self.base_path + f"plots/BB fits/proper_BB_fits/{self.ant_name}/{self.ant_name}_PRESENTATION_subplot_GUIDED_SBB_fits_{self.no_indiv_SED_plots}_({self.individual_BB_plot}).png"
                    else:
    ...
    def plot_SED_params_vs_time(self, band_colour_dict):
        ...
        SBB_T_plot_lims = {'ZTF18aczpgwm': (5e3, 2.2e4), 
                            ...
                            'ZTF22aadesap': (0.3e4, 1.7e4),
                            'PS1-10adi': (None, None), 
                            'ASASSN-17jz': (None, None), 
                            'ASASSN-18jd': (None, None)}

        SBB_R_plot_lims = {'ZTF18aczpgwm': (None, 2.6e16), 
        ...
        if SBB:
            fig, axs = plt.subplots(3, 1, sharex=True, figsize = (9, 13)) # (8.2, 11.6)
            ax1, ax2, ax4 = axs
            ...
            if self.ant_name == 'ZTF18aczpgwm':
                ax1_scalefactor = 1e41
                ax2_scalefactor = 1e16
                ax4_scalefactor = 1e4

            elif self.ant_name == 'ZTF22aadesap':
                ax1_scalefactor = 1e40
                ax2_scalefactor = 1e15
                ax4_scalefactor = 1e4
            ...
            for b in self.interp_df['band'].unique():
                ...
                b_em_cent_wl = b_df['em_cent_wl'].iloc[0]
                b_colour = band_colour_dict[b]
                ax1.errorbar(b_df['d_since_peak'], (b_df['L_rf']/ax1_scalefactor), yerr = (b_df['L_rf_err']/ax1_scalefactor), fmt = 'o', c = b_colour, 
            ...
            scatter_markersize = int(errorbar_markersize**2)
            ...
            ax2.errorbar(BB_N_greater_M['d_since_peak'], (BB_N_greater_M['brute_R_cm']/ax2_scalefactor), yerr = [(BB_N_greater_M['brute_R_err_lower_cm']/ax2_scalefactor), (BB_N_greater_M['brute_R_err_upper_cm']/ax2_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        label = r'N > M and $D_{\sigma_\chi} > 3.0$', fmt = 'o', mec = 'k', mew = '0.9', zorder = 3, markersize = errorbar_markersize, mfc = 'k')
            

            # first plot all fits (good and bad) in grey
            ax2.errorbar(BB_2dp['d_since_peak'], (BB_2dp['brute_R_cm']/ax2_scalefactor), yerr = [(BB_2dp['brute_R_err_lower_cm']/ax2_scalefactor), (BB_2dp['brute_R_err_upper_cm']/ax2_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        fmt = 'o', label = r'N = M and $\chi^2 > 0.1$', mec = 'k', mew = '0.9', mfc = '#696868', zorder = 1, markersize = errorbar_markersize)
            
            # the plot the good fits over the top in white
            ax2.errorbar(BB_2dp_good_fit['d_since_peak'], (BB_2dp_good_fit['brute_R_cm']/ax2_scalefactor), yerr = [(BB_2dp_good_fit['brute_R_err_lower_cm']/ax2_scalefactor), (BB_2dp_good_fit['brute_R_err_upper_cm']/ax2_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        fmt = 'o', label = r'N = M and $\chi^2 \leq 0.1$', mec = 'k', mew = '0.9', zorder = 2, markersize = errorbar_markersize, mfc = 'white')
            ...
            sc = ax2.scatter(BB_low_chi_dist['d_since_peak'], (BB_low_chi_dist['brute_R_cm']/ax2_scalefactor), cmap = 'spring', c = BB_low_chi_dist['abs_brute_chi_sig_dist'].to_numpy(), 
                            label = r'N > M and $D_{\sigma_\chi} \leq 3.0$', marker = 'o', edgecolors = 'k', linewidths = 0.9, zorder = 4, s = scatter_markersize)
            ...
            cbar_label = r'Goodness-of-Fit'
            cbar.set_label(label = cbar_label, fontsize = colorbar_fontsize, fontweight = 'bold')
            ...
            ax2_min, ax2_max = SBB_R_plot_lims[self.ant_name]
            if ax2_min is not None:
                ax2_min = ax2_min / ax2_scalefactor
            if ax2_max is not None:
                ax2_max = ax2_max / ax2_scalefactor
            ...
            ax4.errorbar(BB_N_greater_M['d_since_peak'], (BB_N_greater_M['brute_T_K']/ax4_scalefactor), yerr = [(BB_N_greater_M['brute_T_err_lower_K']/ax4_scalefactor), (BB_N_greater_M['brute_T_err_upper_K']/ax4_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        label = r'N > M and $D_{\sigma_\chi} > 3.0$', marker = 'o', zorder = 3, markersize = errorbar_markersize, mfc = 'k', mec = 'k', mew = '0.9')
            

            # first plot all fits (good and bad) in grey
            ax4.errorbar(BB_2dp['d_since_peak'], (BB_2dp['brute_T_K']/ax4_scalefactor), yerr = [(BB_2dp['brute_T_err_lower_K']/ax4_scalefactor), (BB_2dp['brute_T_err_upper_K']/ax4_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        marker = 'o', label = r'N = M and $\chi^2 > 0.1$', mec = 'k', mew = '0.9', mfc = '#696868', zorder = 1, markersize = errorbar_markersize)
            
            # the plot the good fits over the top in white
            ax4.errorbar(BB_2dp_good_fit['d_since_peak'], (BB_2dp_good_fit['brute_T_K']/ax4_scalefactor), yerr = [(BB_2dp_good_fit['brute_T_err_lower_K']/ax4_scalefactor), (BB_2dp_good_fit['brute_T_err_upper_K']/ax4_scalefactor)], linestyle = 'None', c = '#4f4f4f', 
                        marker = 'o', label = r'N = M and $\chi^2 \leq 0.1$', mec = 'k', mew = '0.9', mfc = 'white', zorder = 2, markersize = errorbar_markersize) #ecolor = 'k'
            

            
            sc = ax4.scatter(BB_low_chi_dist['d_since_peak'], (BB_low_chi_dist['brute_T_K']/ax4_scalefactor), cmap = 'spring', c = BB_low_chi_dist['abs_brute_chi_sig_dist'].to_numpy(), 
                        label = r'N > M and $D_{\sigma_\chi} \leq 3.0$', marker = 'o', edgecolors = 'k', linewidths = 0.9, zorder = 4, s = scatter_markersize)
            ...
            ax4_min, ax4_max = SBB_T_plot_lims[self.ant_name]
            if ax4_min is not None:
                ax4_min = ax4_min / ax4_scalefactor
            if ax4_max is not None:
                ax4_max = ax4_max / ax4_scalefactor
            ...
            ax1.set_ylabel('Luminosity Density\n'+fr'[$\mathbf{{10^{{{int(np.log10(ax1_scalefactor))}}}}}$'+r' erg s$\mathbf{^{-1}}$$\mathbf{\AA^{-1}}$]', fontweight = 'bold', fontsize = subaxis_fontsize)
            ax2.set_ylabel('Blackbody Radius\n'+fr' [$\mathbf{{10^{{{int(np.log10(ax2_scalefactor))}}}}}$ cm]', fontweight = 'bold', fontsize = subaxis_fontsize)
            ax4.set_ylabel('Blackbody Temperature \n'+fr'[$\mathbf{{10^{{{int(np.log10(ax4_scalefactor))}}}}}$ K]', fontweight = 'bold', fontsize = subaxis_fontsize)
            fig.align_ylabels()
            ...
            savepath = self.base_path + f"plots/BB fits/proper_BB_fits/{self.ant_name}/{self.ant_name}_PRESENTATION_SBB_param_vs_DSP.png"
